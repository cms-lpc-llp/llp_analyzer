...
job_script=${RazorAnalyzerDir}/scripts_condor/runRazorJob_JetTiming.sh
filesPerJob=50
...
do
	...
	do
		...
		echo "Arguments = ${analyzer} ${inputfilelist} no 13 ${filesPerJob} ${jobnumber} ${sample}_Job${jobnumber}_Of_${maxjob}.root /store/group/phys_exotica/delayedjets/llp_analyzer/V1p0/MC_Summer16/v2/bkg/${sample} ${analyzerTag} " >> ${jdl_file}
		...
		echo "x509userproxy = /data/jmao/x509_proxy" >> ${jdl_file}
