...
UTILS = $(SRCDIR)/JetCorrectorParameters.cc \
        $(SRCDIR)/SimpleJetCorrectionUncertainty.cc \
		$(SRCDIR)/JetCorrectionUncertainty.cc \
		$(SRCDIR)/SimpleJetCorrector.cc \
		$(SRCDIR)/FactorizedJetCorrector.cc \
		$(SRCDIR)/SimpleJetResolution.cc \
		$(SRCDIR)/BTagCalibrationStandalone.cc \
		$(SRCDIR)/EnergyScaleCorrection_class.cc \
		$(SRCDIR)/EnergyScaleCorrection_class_2017.cc \
		$(SRCDIR)/Hemisphere.cc $(SRCDIR)/Davismt2.cc \
		$(SRCDIR)/RazorHelper.cc \
		$(SRCDIR)/DBSCAN.cc \
		$(SRCDIR)/LiteTreeMuonSystem.cc \
		$(SRCDIR)/LiteLiteTreeMuonSystem.cc \
		$(SRCDIR)/HNLMuonSystemTree.cc \
		${SRCDIR}/TreeMuonSystemCombination.cc \
		${SRCDIR}/TreeMuonSystemBParking.cc \
		${SRCDIR}/TreeMuonSystemBParkingExt.cc \
		${SRCDIR}/TreeMuonSystemDarkShower.cc \
		${SRCDIR}/TreeMuonSystemParticleGun.cc
UTILSOBJ = $(UTILS:cc=o)
