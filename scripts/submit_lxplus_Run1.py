...
################################################
os.system("mkdir -p "+work_directory+"/submission")
os.system("mkdir -p "+work_directory+"/submission/"+process)
os.system("mkdir -p "+work_directory+"/submission/"+process+"/log/")
os.system("mkdir -p "+work_directory+"/submission/"+process+"/input/")
os.system("mkdir -p "+work_directory+"/submission/"+process+"/src/")
os.system("mkdir -p "+work_directory+"/submission/"+process+"/out/")
#######################################

submitjobs_name = work_directory+"/submission/"+process+"/src/send_allfiles.sh"
submitjobs = open(submitjobs_name, 'w')
submitjobs.write("#!/bin/bash \n\n") 

for ijob in range(njobs):
    # prepare the list file
    inputfilename = work_directory+"/submission/"+process+"/input/input_"+str(ijob)+".list"
    ...
    outputName = work_directory + "/submission/" + process + "/src/submit_" + str(ijob) + ".sh"
    rootOutput = work_directory + "/submission/" + process + "/out/" + process + "_" + str(ijob) + ".root"
    logName = work_directory + "/submission/" + process + "/log/log_" + str(ijob)
    errName = work_directory + "/submission/" + process + "/log/err_" + str(ijob)
    outputfile = open(outputName,'w')
    outputfile.write("#!/bin/sh\n\n")
    
    outputfile.write( "cd " + work_directory )
    #outputfile.write( "\npwd")
    outputfile.write("\nulimit -c 0\n")
    ...
    #outputfile.write( "\npwd")
    if _isData:
        outputfile.write( "./RazorRun " + inputfilename  + " " + appName + " --isData" + " -f=" + rootOutput + " -n=" + option )
    else :
        outputfile.write( "./RazorRun " + inputfilename  + " " + appName + " -f=" + rootOutput + " -n=" + option )
        
    outputfile.close
    print "bsub -q 1nh -e " +errName+ " -o " +logName + "  source " + outputName
    os.system("bsub -q 1nh -e " +errName+ " -o " +logName + "  source " + outputName)
    os.system("sleep 1\n")
    scriptfile=work_directory+"/"+outputName
    ijob = ijob+1
    continue
