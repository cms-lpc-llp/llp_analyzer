...
//Analyze
void SusyLLPPassFail::Analyze(bool isData, int options, string outputfilename, string analysisTag, string process)
{
	...
	if (outfilename == "") outfilename = "SusyLLPPassFailTree.root";
	TFile *outFile = new TFile(outfilename.c_str(), "RECREATE");
	//RazorLiteTree *llp_tree = new RazorLiteTree;
	SusyLLPPassFailTree *llp_tree = new SusyLLPPassFailTree;
	llp_tree->CreateTree();
	...
	for (Long64_t jentry=0; jentry<fChain->GetEntries();jentry++) {
		...
		if( (label.find("MR_EMU") != std::string::npos) && llp_tree->nMuons != 1 ) continue;
		if( (label.find("MR_JetHT") != std::string::npos) && llp_tree->nMuons != 0 ) continue;
		...
		float dPhi = RazorAnalyzerLLP::deltaPhi(metType1Phi, lepp4.Phi());
		//float dPhi = RazorAnalyzerLLP::deltaPhi(llp_tree->metPhi, lepp4.Phi());
		//llp_tree->MT = sqrt(2*(llp_tree->met)*lepp4.Pt()*(1-cos(dPhi)));
		//llp_tree->MT = sqrt(2*(metType1Pt)*lepp4.Pt()*(1-cos(dPhi)));
		float MT = sqrt(2*(metType1Pt)*lepp4.Pt()*(1-cos(dPhi)));
		...
		//if( (label.find("MR_Single") != std::string::npos) && llp_tree->MT>=100 ) continue;
		...
		for(int i = 0; i < nJets; i++)
		{
			...
			jets tmpJet;
			tmpJet.jet    = thisJet;
			if(jetRechitT[i]>0.09 && jetMinDeltaRPVTracks[i]>0.06 && jetChargedHadronEnergyFraction[i]<0.06 && jetGammaMax_ET[i]<0.16)
			{
				tmpJet.PassFail = 1; 
			}
			else
		...
		//if( (label=="MR_PHO") && llp_tree->jetPho_dPhiMin <=2.8 ) continue;
