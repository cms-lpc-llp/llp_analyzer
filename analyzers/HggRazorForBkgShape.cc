...
void HggRazorForBkgShape::Analyze(bool isData, int option, string outFileName, string label)
{
  //*****************************************************************************
  //Settings
  //*****************************************************************************
  ...
  string analysisTag = "";
  if (option >= 0 && option <= 10) analysisTag = "2015_76X";
  else if (option >= 11 && option <= 20) analysisTag = "2016_80X";
  else {
  ...
  //option 8 / 18: LooseID, TightIso
  //option 9 / 19: TightID , TightIso
  //option 10 / 20: Regular Selection, inverted isolation
  ...
  std::cout << "[INFO]: doInverseIso --> " << doInverseIso << std::endl;
  ...
  if ( outFileName.empty() )
    {
      if ( _info ) std::cout << "HggRazorForBkgShape: Output filename not specified!" << endl << "Using default output name HggRazor.root" << std::endl;
...
*/
  ...
  for ( Long64_t jentry=0; jentry < nentries; jentry++ )
      ...
      for(int i = 0; i < nPhotons; i++)
	{
	  ...
	  if (doRequireIso) {
	    if ((doInverseIso) && (photonPassLooseIso(i))) continue;
	    if (!(doInverseIso) && !(photonPassLooseIso(i))) continue;
	  }
	  if (doRequireTightIso) {
	    if ((doInverseIso)&&(photonPassTightIso(i))) continue;
	    if (!(doInverseIso)&&!(photonPassTightIso(i))) continue;
	  }	  	  	  
