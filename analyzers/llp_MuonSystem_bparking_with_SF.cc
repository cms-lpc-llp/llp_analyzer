#include "llp_MuonSystem_bparking_with_SF.h"
...
void llp_MuonSystem_bparking_with_SF::Analyze(bool isData, int options, string outputfilename, string analysisTag)
{
  ...
  for (Long64_t jentry=0; jentry<fChain->GetEntries();jentry++) {
    ...
    for(int i = 0; i < nGenParticle;i++)
    {
      ...
      if (gParticleMotherIndex[i] == llp_mother) {
        printf("Gen Particle: %10i %10i %10i %10f %10f %10f \n", gParticleId[i], gParticleMotherId[i], gParticleMotherIndex[i], gParticlePt[i], gParticleEta[i], gParticlePhi[i]);
    ...
    //if (!(MuonSystem->gLLP_dt || MuonSystem->gLLP_csc))continue;
          //*************************************************************************
	  ...
	  if (isData)
	   {
		...
		if (tmp.tWeighted>12 || tmp.tWeighted<-5)continue;
		if (abs(tmp.eta)>2.2)continue;
	  }
	  */
	  ...
	  cout<<"MuonSystem->dtRechitClusterMatchedTrackSumPt_0p2: "<<MuonSystem->dtRechitClusterMatchedTrackSumPt_0p2[MuonSystem->nDtRechitClusters]<<endl;
	  cout<<"MuonSystem->dtRechitClusterMatchedTrackSumPt_0p3: "<<MuonSystem->dtRechitClusterMatchedTrackSumPt_0p3[MuonSystem->nDtRechitClusters]<<endl;
	  cout<<"MuonSystem->dtRechitClusterMatchedTrackSumPt_0p4: "<<MuonSystem->dtRechitClusterMatchedTrackSumPt_0p4[MuonSystem->nDtRechitClusters]<<endl;
	  cout<<"MuonSystem->dtRechitClusterMatchedTrackSumPt_0p5: "<<MuonSystem->dtRechitClusterMatchedTrackSumPt_0p5[MuonSystem->nDtRechitClusters]<<endl;
