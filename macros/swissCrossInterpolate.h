...
TH2D* swissCrossInterpolate(const TH2D* hist, std::string firstInterpolationDirection);
// increases binning by factor of two and interpolates in specified direction
TH2D* swissCrossRebin(TH2D* hist, std::string firstInterpolationDirection);

void getHistMaxMinBins(TH2D* h, int &xMin, int &xMax, int &yMin, int &yMax);
void omitEven(TH2D*h);
void omitOdd(TH2D*h);
bool alongDiagonal(TH2D* h, int iX, int iY);
...
TH2D* swissCrossInterpolate(const TH2D* hist, std::string firstInterpolationDirection) 
{
  ...
  TH2D *hist_step1 = (TH2D*)histCopy->Clone();
  hist_step1->Reset();
  TH2D *hist_step2 = (TH2D*)histCopy->Clone();
  hist_step2->Reset();
...
TH2D* swissCrossRebin(TH2D* hist, std::string firstInterpolationDirection)
{
  ...
  TH2D * histRebinned = new TH2D(histName, histName,
			  2*hist->GetNbinsX(),
  ...
  TH2D* histRebinnedInterpolated = swissCrossInterpolate(histRebinned, firstInterpolationDirection);
