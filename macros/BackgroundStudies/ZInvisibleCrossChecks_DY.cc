...
void ZInvisibleCrossChecks(){
    ...
    mcfiles["TTJetsDY"] = new TFile("./TTJetsRun1_19700pb_weighted.root");
    mcfiles["TopDY"] = new TFile("./SingleTopRun1_19700pb_weighted.root");
    mcfiles["TTWDY"] = new TFile("./TTWJetsRun1_19700pb_weighted.root");
    mcfiles["TTZDY"] = new TFile("./TTZJetsRun1_19700pb_weighted.root");
    ...
    for(auto &file : mcfiles){
        ...
        mctrees[file.first]->SetBranchStatus("HT_noZ", 1);
        mctrees[file.first]->SetBranchStatus("numJets80_noZ", 1);
        mctrees[file.first]->SetBranchStatus("numJets_noZ", 1);
    ...
    for(auto &file : datafiles){
        ...
        datatrees[file.first]->SetBranchStatus("HT_noZ", 1);
        datatrees[file.first]->SetBranchStatus("numJets80_noZ", 1);
        datatrees[file.first]->SetBranchStatus("numJets_noZ", 1);
        ...
        datatrees[file.first]->SetBranchAddress("nLooseMuons", &nLooseMuons);
    ...
    cutSequence.push_back( "nLooseMuons == 2 && hlt_dimuon && recoZmass > 71 && recoZmass < 111 && MR_noZ > 300 && Rsq_noZ > 0.15 && numJets80_noZ > 1" );
    cutName.push_back( "Z+Jets Control Sample" );
    ...
    colors["DYJets"] = kAzure;
    colors["TopDY"] = kViolet-5;
    colors["TTJetsDY"] = kViolet-6;
    colors["TTWDY"] = kRed+2;
    colors["TTZDY"] = kOrange-3;
    TLegend *legend = new TLegend(0.7, 0.7, 0.9, 0.9);
    legend->AddEntry(dataNJets[0], "2012 Data, Double Muon CS");
    legend->AddEntry(mcNJets["DYJets"][0], "DY+Jets MC");
    legend->AddEntry(mcNJets["TTJetsDY"][0], "TTJets MC");
    legend->AddEntry(mcNJets["TopDY"][0], "Single Top MC");
    legend->AddEntry(mcNJets["TTWDY"][0], "TTW MC");
    legend->AddEntry(mcNJets["TTZDY"][0], "TTZ MC");

    cout<<"DY "<<mcNJets["DYJets"][0]->Integral()<<endl;
    cout<<"TTJetsDY "<<mcNJets["TTJetsDY"][0]->Integral()<<endl;
    cout<<"TopDY "<<mcNJets["TopDY"][0]->Integral()<<endl;
    cout<<"TTWDY "<<mcNJets["TTWDY"][0]->Integral()<<endl;
    cout<<"TTZDY "<<mcNJets["TTZDY"][0]->Integral()<<endl;
    ...
    for(uint cut = 0; cut < cutSequence.size(); cut++){
        ...
        vector<string> orderedtrees {"TTZDY","TTWDY","TopDY","TTJetsDY","DYJets"};
        ...
        DrawDataVsMCRatioPlot(dataNJets[cut], &NumJetsMC, legend, "Number of jets 40 GeV", "DYJetsCrossChecks_NumJets"+to_string(cut), false);
        DrawDataVsMCRatioPlot(dataNJets80[cut], &NumJets80MC, legend, "Number of jets 80 GeV", "DYJetsCrossChecks_NumJets80"+to_string(cut), false);
        DrawDataVsMCRatioPlot(dataMR[cut], &MRMC, legend, "MR (GeV)", "DYJetsCrossChecks_MR"+to_string(cut), true);
        DrawDataVsMCRatioPlot(dataRsq[cut], &RsqMC, legend, "Rsq", "DYJetsCrossChecks_Rsq"+to_string(cut), false);
        DrawDataVsMCRatioPlot(dataMet[cut], &MetMC, legend, "Met", "DYJetsCrossChecks_MET"+to_string(cut), false);
        DrawDataVsMCRatioPlot(dataNvtx[cut], &NVtxMC, legend, "NVtx", "DYJetsCrossChecks_NVtx"+to_string(cut), false);
        DrawDataVsMCRatioPlot(dataHT[cut], &HTMC, legend, "HT", "DYJetsCrossChecks_HT"+to_string(cut), false);
    }
    ...
    mcfiles["DYJets"]->Close();
    mcfiles["TopDY"]->Close();
    mcfiles["TTJetsDY"]->Close();
    mcfiles["TTWDY"]->Close();
    mcfiles["TTZDY"]->Close();
    datafiles["DYJets"]->Close();
    pileupWeightFile->Close();
    delete mcfiles["DYJets"];
    delete mcfiles["TopDY"];
    delete mcfiles["TTJetsDY"];
    delete mcfiles["TTWDY"];
    delete mcfiles["TTZDY"];
    delete datafiles["DYJets"];
