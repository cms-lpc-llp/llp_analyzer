...
def make_tf2_sevenjet():
    box = 'SevenJet'
    names = ['Intercept', 'Y slope', 'X slope']
    multipliers = [1, 1, 1000]
    num_pars = len(names)
    pars = range(num_pars)
    function_expr = "[{}] + [{}]*y + [{}]*x".format(*pars)
...
def make_tf2(box):
    ...
    names = ['Intercept', 'X slope', 'Y slope', 'XY slope', 'X^2 slope']
    multipliers = [1, 1000, 1, 1000, 1000000]
    num_pars = len(names)
    pars = range(num_pars)
    function_expr = "[{}] + [{}]*x + ([{}] + [{}]*x)*y + [{}]*(x - 650)^2".format(*pars)
    print "Fit function: "+function_expr
    return (rt.TF2("qcd{}".format(box.lower()), function_expr), 
            num_pars, names, multipliers)

def export_fit(fun, result, box):
    out_name = 'qcd_best_fit_2d_{}_quadratic.root'.format(box.lower())
