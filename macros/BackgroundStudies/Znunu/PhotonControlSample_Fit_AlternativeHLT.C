...
void RunPhotonControlSample_Fit(  vector<string> datafiles, vector<vector<string> > bkgfiles, vector<string> bkgLabels, vector<int> bkgColors, double lumi, string option, int channelOption = -1, string label = "") {
  ...
  TFile *PromptFile = new TFile("PromptPhotonTemplate_EB_VarBins_AlternativeHLT.root", "READ");
  TH1F *promptHist = (TH1F*)PromptFile->Get("h1");
 
  TFile *PromptFile_EE = new TFile("PromptPhotonTemplate_EE_VarBins_AlternativeHLT.root", "READ");
	...
	bool passTrigger = false;
        ...
        if (isData) {
          ...
          if (events->pho1.Pt() > 185) {
            dataWeight = 1;
            if (events->HLTDecision[83] && events->pho1HLTFilter[30]) passTrigger = true;
          }
          else if (events->pho1.Pt() > 135) {
            dataWeight = events->HLTPrescale[81];
            if (events->HLTDecision[81] && events->pho1HLTFilter[32]) passTrigger = true;
          } else if (events->pho1.Pt() > 105) {
            dataWeight = events->HLTPrescale[80];
            if (events->HLTDecision[80] && events->pho1HLTFilter[33]) passTrigger = true;
          } else if (events->pho1.Pt() > 85) {
            dataWeight = events->HLTPrescale[79];
            if (events->HLTDecision[79] && events->pho1HLTFilter[34]) passTrigger = true;
          } else if (events->pho1.Pt() > 58){
            dataWeight = events->HLTPrescale[78];
            if (events->HLTDecision[78] && events->pho1HLTFilter[35]) passTrigger = true;
          } else {
            dataWeight = events->HLTPrescale[77];
            if (events->HLTDecision[77] && events->pho1HLTFilter[36]) passTrigger = true;
          }
          ...
          if (
               events->HLTDecision[77] || events->HLTDecision[78] || events->HLTDecision[79]
               || events->HLTDecision[80] || events->HLTDecision[81] ||
              events->HLTDecision[83]
              ) passTrigger = true;
        }
...
void PhotonControlSample_Fit_AlternativeHLT( int option = 10) {
...
chain->Draw("pho1_sigmaietaieta>>h1","weight*((HLTDecision[77]==1 || HLTDecision[78]==1 || HLTDecision[79]==1 || HLTDecision[80] ==1 ||HLTDecision[81]==1 || HLTDecision[83]==1)&&fabs(pho1.Eta())<1.479&&pho1_chargediso<2.5&&pho1_sigmaietaieta<0.015)");
h1->SaveAs("a.root")
