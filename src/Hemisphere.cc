...
int Hemisphere::Reconstruct(){
    ...
    if (seed_meth == 1) {
        ...
        for (int i = 0; i < vsize; ++i){
            if( i == I_Max) continue;
            float DeltaR = sqrt((Object_Eta[i] - Object_Eta[I_Max])*(Object_Eta[i] - Object_Eta[I_Max]) + deltaPhi(Object_Phi[i], Object_Phi[I_Max])*deltaPhi(Object_Phi[i], Object_Phi[I_Max]));
