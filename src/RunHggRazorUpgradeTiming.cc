...
void usage()
{
  std::cerr << "Usage: RunHggRazorUpgradeTiming  <input list>  [options]\n[options]:\n"
	    << "-d  --isData\n"
	    << "-t  --useTiming\n"
	    << "-c  --usePhoChi2\n"
	    << "-e  --useOddEvent\n"
...
int main(int argc, char* argv[]){
  ...
  std::string _useTiming = ParseCommandLine( argc, argv, "--useTiming" );
  std::string _usePhoChi2 = ParseCommandLine( argc, argv, "--usePhoChi2" );
  std::string _useOddEvent = ParseCommandLine( argc, argv, "--useOddEvent" );
  std::string _t = ParseCommandLine( argc, argv, "-t" );
  std::string _c = ParseCommandLine( argc, argv, "-c" );
  std::string _e = ParseCommandLine( argc, argv, "-e" );
  bool useTiming = false;
  bool usePhoChi2 = false;
  bool useOddEvent = false;
  if ( _useTiming == "yes" || _t == "yes" ) useTiming = true;
  if ( _usePhoChi2 == "yes" || _c == "yes" ) usePhoChi2 = true;
  if ( _useOddEvent == "yes" || _e == "yes" ) useOddEvent = true;
  ...
  std::cout << "[INFO]: useTiming --> " << useTiming << std::endl;
  std::cout << "[INFO]: usePhoChi2 --> " << usePhoChi2 << std::endl;
  std::cout << "[INFO]: useOddEvent --> " << useOddEvent << std::endl;
  ...
  std::cout << "[INFO]: optionalLabel --> " << label << std::endl;
    ...
    HggRazorUpgradeTiming analyzer(theChain);
    
    //------ EXECUTE ------//
    cout << "Executing HggRazorUpgradeTiming..." << endl;
    analyzer.EnableAll();
    analyzer.Analyze(isData, useTiming, usePhoChi2, useOddEvent, option, outputFileName, label);
