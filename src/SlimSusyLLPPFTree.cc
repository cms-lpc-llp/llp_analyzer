...
// Constructor
SlimSusyLLPPFTree::SlimSusyLLPPFTree()
{
	InitVariables();
};
...
//Initialize
void SlimSusyLLPPFTree::InitVariables()
{ 
	//evnt info
	lheComments=0;
	mH=0; mX=0; ctau=0;
...
//SetBranchAddress
void SlimSusyLLPPFTree::InitTree()
{
	assert(tree_);
	InitVariables();
...
//LoadTree
void SlimSusyLLPPFTree::LoadTree(const char* file)
{
	f_ = TFile::Open(file);
	assert(f_);
	tree_ = dynamic_cast<TTree*>(f_->Get("SlimSusyLLPPFTree"));
	InitTree();
	assert(tree_);
};

//CreateTree
void SlimSusyLLPPFTree::CreateTree()
{
	//tree
	tree_ = new TTree("SlimSusyLLPPFTree","SlimSusyLLPPFTree");
	f_ = 0;
