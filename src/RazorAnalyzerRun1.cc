...
float EB_GetPFchHadIsoCut_T( ){ return 0.91; };
...
float EB_GetPFnHadIsoCut_T( float pt ){ return (0.33 + EB_PFnHadIsoSlope*pt); };
...
float EB_GetPFphoIsoCut_T( float pt ){ return (0.61 + EB_PFphoIsoSlope*pt); };
...
float EE_GetPFchHadIsoCut_T( ){ return 0.65; };
...
float EE_GetPFnHadIsoCut_T( float pt ){ return (0.93 + EE_PFnHadIsoSlope*pt); };

float EE_GetPFphoIsoCut_M( float pt ){ return (1.0 + 0.005*pt); };
float EE_GetPFphoIsoCut_T( float pt ){ return (0.54 + 0.005*pt); };



RazorAnalyzerRun1::RazorAnalyzerRun1(TTree *tree) : RazorEventsRun1(tree)
{
    //turn off all branches
    fChain->SetBranchStatus("*", 0);
}

RazorAnalyzerRun1::~RazorAnalyzerRun1()
{

}

void RazorAnalyzerRun1::Analyze(bool isData, int option, string outputFileName, string label) {
    cout << "Analyze method called on base RazorAnalyzerRun1 instance.  Parameters were: " << isData << " " << option << " " << outputFileName << " " << label << endl;
}
...
void RazorAnalyzerRun1::EnableEventInfo(){
...
void RazorAnalyzerRun1::EnableMuons(){
...
void RazorAnalyzerRun1::EnableIsoPFCandidates(){
...
void RazorAnalyzerRun1::EnablePhotons(){
...
void RazorAnalyzerRun1::EnableMC(){
...
float RazorAnalyzerRun1::GetElectronScaleCorrection( double pt, double eta ) {  
...
float RazorAnalyzerRun1::GetElectronEffectiveAreaMean(int i, bool use25nsCuts ){ 
...
float RazorAnalyzerRun1::GetElectronEffectiveArea90(int i){ 
...
bool RazorAnalyzerRun1::isEGammaPOGVetoElectron(int i, bool applyID, bool applyIso, bool use25nsCuts){
  bool pass = true;
  if (applyID) {
...
bool RazorAnalyzerRun1::isEGammaPOGLooseElectron(int i, bool applyID, bool applyIso, bool use25nsCuts){
  bool pass = true;
  if (applyID) {
...
bool RazorAnalyzerRun1::isEGammaPOGMediumElectron(int i, bool applyID, bool applyIso, bool use25nsCuts){
  bool pass = true;
  if (applyID) {
...
bool RazorAnalyzerRun1::isEGammaPOGTightElectron(int i, bool applyID, bool applyIso, bool use25nsCuts){
  bool pass = true;
  if (applyID) {
...
bool RazorAnalyzerRun1::passEGammaPOGLooseElectronID(int i, bool use25nsCuts){
  bool pass = false;
...
bool RazorAnalyzerRun1::passEGammaPOGMediumElectronID(int i, bool use25nsCuts){
  bool pass = false;
...
bool RazorAnalyzerRun1::passEGammaPOGTightElectronID(int i, bool use25nsCuts){
  bool pass = false;
...
bool RazorAnalyzerRun1::passEGammaPOGLooseElectronIso(int i, bool use25nsCuts){
  bool pass = false;
...
bool RazorAnalyzerRun1::passEGammaPOGMediumElectronIso(int i, bool use25nsCuts){
  bool pass = false;
...
bool RazorAnalyzerRun1::passEGammaPOGTightElectronIso(int i, bool use25nsCuts){
  bool pass = false;
...
bool RazorAnalyzerRun1::isHZZElectron(int i){
...
bool RazorAnalyzerRun1::matchElectronHLTFilters2015(int i, string HLTFilter){
  bool match = false;
...
float RazorAnalyzerRun1::GetMuonEffectiveAreaMean(int i, string type ){ 
...
bool RazorAnalyzerRun1::isMuonPOGLooseMuon(int i, bool applyID, bool applyIso){
  bool pass = true;
  if (applyID) {
...
bool RazorAnalyzerRun1::isLooseMuon(int i, bool applyID, bool applyIso){
  bool pass = true;
  double dr = fmax(0.05,fmin(0.2, 10/muonPt[i]));
  if (applyID) {
    if (!(muonIsLoose[i] && fabs(muon_ip3dSignificance[i]) < 4)) pass = false;
...
bool RazorAnalyzerRun1::isTightMuon(int i, bool applyID, bool applyIso){
  bool pass = true;
  double dr = fmax(0.05,fmin(0.2, 10/muonPt[i]));
  if (applyID) {
...
bool RazorAnalyzerRun1::passHZZMuonPreselection(int i){
  bool pass = false;
  if (muonIsLoose[i] 
      && fabs(muon_d0[i]) < 0.5
      && fabs(muon_dZ[i]) < 1.0
...
bool RazorAnalyzerRun1::isHZZMuon(int i){
  bool pass = false;
  if (muonIsLoose[i] 
      && fabs(muon_d0[i]) < 0.5
      && fabs(muon_dZ[i]) < 1.0
...
bool RazorAnalyzerRun1::isLooseTau(int i){
  bool pass = false;
  if (tau_IsLoose[i]) {
    pass = true;
  }
...
bool RazorAnalyzerRun1::isMediumTau(int i){
  bool pass = false;
  if (tau_IsMedium[i]) {
    pass = true;
  }
...
bool RazorAnalyzerRun1::isTightTau(int i){
  bool pass = false;
  if (tau_IsTight[i]) {
    pass = true;
  }
...
bool RazorAnalyzerRun1::isCSVL(int i, string dataset){
  if (dataset == "74X") {
...
bool RazorAnalyzerRun1::isCSVM(int i, string dataset){
  if (dataset == "74X") {
...
bool RazorAnalyzerRun1::isCSVT(int i, string dataset){
  if (dataset == "74X") {
...
double RazorAnalyzerRun1::JetEnergyCorrectionFactor( double jetRawPt, double jetEta, double jetPhi, double jetE,
...
double RazorAnalyzerRun1::JetEnergySmearingFactor( double jetPt, double jetEta, double NPU, SimpleJetResolution *JetResolutionCalculator, TRandom3 *random ) {
...
double RazorAnalyzerRun1::UpDownJetEnergySmearingFactor(double unsmearedPt, double jetEta, double NPU, SimpleJetResolution *JetResolutionCalculator, double smearedPt, string option){
    //get jet resolution
    std::vector<float> fJetEta, fJetPtNPU;
    fJetEta.push_back(jetEta);  
...
double RazorAnalyzerRun1::BTagScaleFactor(double jetPt, bool CSVM, string option){
...
bool RazorAnalyzerRun1::photonPassesElectronVeto(int i){
    //use presence of a pixel seed as proxy for an electron veto
    return (pho_passEleVeto[i]);
}


void RazorAnalyzerRun1::getPhotonEffAreaRun2( float eta, double& effAreaChHad, double& effAreaNHad, double& effAreaPho )
{
  if( fabs( eta ) < 1.0 )
    {
      effAreaChHad = 0.0157;
...
void RazorAnalyzerRun1::getPhotonEffArea90( float eta, double& effAreaChHad, double& effAreaNHad, double& effAreaPho )
{
  if( fabs( eta ) < 1.0 )
    {
      effAreaChHad = 0.0456;
...
bool RazorAnalyzerRun1::photonPassMediumIDWithoutEleVeto(int i, bool use25nsCuts){
  bool pass = true;
...
bool RazorAnalyzerRun1::photonPassTightIDWithoutEleVeto(int i, bool use25nsCuts){
  bool pass = true;
...
bool RazorAnalyzerRun1::photonPassLooseID(int i, bool use25nsCuts){
...
bool RazorAnalyzerRun1::photonPassMediumID(int i, bool use25nsCuts){
  bool pass = true;
...
bool RazorAnalyzerRun1::photonPassTightID(int i, bool use25nsCuts){
  bool pass = true;
...
bool RazorAnalyzerRun1::photonPassMediumIso(int i, bool use25nsCuts){
...
bool RazorAnalyzerRun1::photonPassTightIso(int i, bool use25nsCuts){
...
bool RazorAnalyzerRun1::isLoosePhotonWithoutEleVeto(int i, bool use25nsCuts){
  ...
  if(!photonPassIsoRun1( i ,  false )) pass = false;
...
bool RazorAnalyzerRun1::photonPassLooseIDWithoutEleVetoExo15004( int i )
{
  bool pass = true;
...
int RazorAnalyzerRun1::findClosestGenElectron(double eta, double phi) {
  int matchedIndex = -1;
  float minDR = 999.0;
...
int RazorAnalyzerRun1::findClosestGenMuon(double eta, double phi) {
  int matchedIndex = -1;
  float minDR = 999.0;
  for(int j = 0; j < nGenParticle; j++){
    if (gParticleStatus[j] != 1) continue;
...
int RazorAnalyzerRun1::findClosestGenJet(double eta, double phi) {
  int matchedIndex = -1;
  float minDR = 999.0;
...
bool RazorAnalyzerRun1::isGenTau(int index){
...
//Checks if the gParticle is a tau and that comes from a W or a Z
bool RazorAnalyzerRun1::isGenLeptonicTau(int index){
...
int RazorAnalyzerRun1::findClosestGenTau(double eta, double phi) {
  int matchedIndex = -1;
  float minDR = 999.0;
  for(int j = 0; j < nGenParticle; j++){
...
int RazorAnalyzerRun1::findClosestRecoTau(double eta, double phi) {
  int matchedIndex = -1;
  float minDR = 999.0;
...
int RazorAnalyzerRun1::GetTauMatchedID(double eta, double phi){
  int matchedID = 0;
...
int RazorAnalyzerRun1::findClosestParton(float eta, float phi){
...
bool RazorAnalyzerRun1::matchesGenMuon(double eta, double phi){
  bool result = false;
  for(int j = 0; j < nGenParticle; j++){
    if ( deltaR(eta, phi, gParticleEta[j], gParticlePhi[j]) < 0.1 && 
...
bool RazorAnalyzerRun1::matchesGenElectron(double eta, double phi){
  bool result = false;
  for(int j = 0; j < nGenParticle; j++){
    if ( deltaR(eta, phi, gParticleEta[j], gParticlePhi[j]) < 0.1 && 
...
double RazorAnalyzerRun1::deltaPhi(double phi1, double phi2) {
...
double RazorAnalyzerRun1::deltaR(double eta1, double phi1, double eta2, double phi2) {
  double dphi = deltaPhi(phi1,phi2);
  double deta = eta1 - eta2;
  return sqrt( dphi*dphi + deta*deta);
}

TLorentzVector RazorAnalyzerRun1::makeTLorentzVector(double pt, double eta, double phi, double energy){
    TLorentzVector vec;
    vec.SetPtEtaPhiE(pt, eta, phi, energy);
    return vec;
}

TLorentzVector RazorAnalyzerRun1::makeTLorentzVectorPtEtaPhiM(double pt, double eta, double phi, double mass){
    TLorentzVector vec;
    vec.SetPtEtaPhiM(pt, eta, phi, mass);
    return vec;
}

double RazorAnalyzerRun1::GetAlphaT(vector<TLorentzVector> jets) 
{   
    int nJets = jets.size();
...
double RazorAnalyzerRun1::GetDPhiMin(vector<TLorentzVector> jets)
...
vector<TLorentzVector> RazorAnalyzerRun1::getHemispheres(vector<TLorentzVector> jets){
    int nJets = jets.size();
    vector<TLorentzVector> possibleHem1s; //holds possible hemisphere combinations
    vector<TLorentzVector> possibleHem2s;
...
std::vector< std::vector<int> > RazorAnalyzerRun1::getHemispheresV2( std::vector<TLorentzVector> jets )
{
...
double RazorAnalyzerRun1::computeMR(TLorentzVector hem1, TLorentzVector hem2){
    return sqrt(pow(hem1.P() + hem2.P(), 2) - pow(hem1.Pz() + hem2.Pz(), 2));
}

double RazorAnalyzerRun1::computeRsq(TLorentzVector hem1, TLorentzVector hem2, TLorentzVector pfMet){
...
double RazorAnalyzerRun1::GetMT( TLorentzVector visible, TVector3 met )
{
...
double RazorAnalyzerRun1::GetMT( TLorentzVector visible, TLorentzVector met )
{
...
double RazorAnalyzerRun1::GetMTEnergy( TLorentzVector visible, TVector3 met )
{
  TVector3 vis( visible.Px(), visible.Py(), visible.Pz() );
...
double RazorAnalyzerRun1::GetMTEnergy( TLorentzVector visible, TLorentzVector met )
{
  TVector3 _met( met.Px(), met.Py(), met.Pz() );
...
double RazorAnalyzerRun1::GetDphi( TLorentzVector visible, TVector3 met )
{
  TVector3 vis( visible.Px(), visible.Py(), visible.Pz() );
  return vis.DeltaPhi( met );
};

double RazorAnalyzerRun1::GetDphi( TLorentzVector visible, TLorentzVector met )
{
  TVector3 _met( met.Px(), met.Py(), met.Pz() );
...
bool RazorAnalyzerRun1::passesHadronicRazorBaseline(double MR, double Rsq){
    bool passes = true;
...
bool RazorAnalyzerRun1::passesLeptonicRazorBaseline(double MR, double Rsq){
    bool passes = true;
    if(MR < 0 || Rsq < 0) passes = false;
    //temporarily disable these
...
int RazorAnalyzerRun1::SubtractParticleFromCollection(TLorentzVector ToSubtract, vector<TLorentzVector>& Collection, float deltaRMatch){
...
double RazorAnalyzerRun1::calcMT2(float testMass, bool massive, vector<TLorentzVector> jets, TLorentzVector MET, int hemi_seed, int hemi_association)
{
...
bool RazorAnalyzerRun1::passEleVetoRun1( int i )
{
  return pho_passEleVeto[i];
};
...
/*
//photon ID and isolation cuts from https://twiki.cern.ch/twiki/bin/viewauth/CMS/EgammaIDRecipesRun2
bool RazorAnalyzerRun1::photonPassesIsolation(int i, double PFChHadIsoCut, double PFNeuHadIsoCut, double PFPhotIsoCut){
    //get effective area for isolation calculations
    double effAreaChargedHadrons = 0.0;
    double effAreaNeutralHadrons = 0.0;
    double effAreaPhotons = 0.0;
...
*/
void RazorAnalyzerRun1::getPhotonEffAreaRun1( float eta, double& effAreaChHad, double& effAreaNHad, double& effAreaPho )
{
  if( fabs( eta ) < 1.0 )
    {
      effAreaChHad = 0.012;
...
//----------------------------------------
bool RazorAnalyzerRun1::photonPassIsoRun1( int i , bool _debug )
{
  //Define variables
  ...
  if ( _isEB )
    {
      if ( PFIsoCorrected_chHad > EB_PFchHadIsoCut )
	{
	  if ( _debug ) std::cout << "EB, Iso: failed chHadIso, cut @ " << EB_PFchHadIsoCut << std::endl;
	  return false;
	}
      if ( PFIsoCorrected_nHad > (EB_PFnHadIsoConst + EB_PFnHadIsoSlope*pt) )
	{
	  if ( _debug ) std::cout << "EB, Iso: failed nHadIso, cut @ " << (EB_PFnHadIsoConst + EB_PFnHadIsoSlope*pt) << std::endl;
	  return false;
	}
      if ( PFIsoCorrected_pho > (EB_PFphoIsoConst + EB_PFphoIsoSlope*pt) )
	{
	  if ( _debug ) std::cout << "EB, Iso: failed phoIso, cut @ " << (EB_PFphoIsoConst + EB_PFphoIsoSlope*pt) << std::endl;
	  return false;
	}
    }
  else
    {
      if ( PFIsoCorrected_chHad > EE_PFchHadIsoCut )
	{
	  if ( _debug ) std::cout << "EE, Iso: failed chHadIso, cut @ " << EE_PFchHadIsoCut << std::endl;
	  return false;
	}
      if ( PFIsoCorrected_nHad > (EE_PFnHadIsoConst + EE_PFnHadIsoSlope*pt) )
	{
	  if ( _debug ) std::cout << "EE, Iso: failed nHadIso, cut @ " << (EE_PFnHadIsoConst + EE_PFnHadIsoSlope*pt) << std::endl;
	  return false;
	}
    }
...
bool RazorAnalyzerRun1::photonPassIsoRun1( int i , WP wp, bool _debug )
{
  //Define variables                                                                                
  float eta = pho_superClusterEta[i];
  float pt  = phoPt[i];//default pt                                          
  ...
  if ( wp == WP::VeryLoose )
    ...
    {
      _EB_chHadCut = EB_GetPFchHadIsoCut_T();
      _EB_nHadCut  = EB_GetPFnHadIsoCut_T( pt );
      _EB_phoCut   = EB_GetPFphoIsoCut_T( pt );
      _EE_chHadCut = EE_GetPFchHadIsoCut_T();
      _EE_nHadCut  = EE_GetPFnHadIsoCut_T( pt );
      _EE_phoCut   = EE_GetPFphoIsoCut_T( pt );
    }
...
bool RazorAnalyzerRun1::isGoodPhotonRun1( int i, bool _iso = false, bool _debug = false)
{
  bool _isEB = false;
