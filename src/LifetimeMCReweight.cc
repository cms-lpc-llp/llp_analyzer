...
double getNormalizationWeight(string filename, string datasetName, double ctau_old, double ctau_new) {
  ...
  double Weight = exp(1.0/ctau_old - 1.0/ctau_new);
...
int main(int argc, char* argv[]) {
    ...
    if(argc < 4){
        cerr << "usage NormalizeNtuple inputList.txt <original ctau> <new ctau>" << endl;
        return -1;
    }
    string inputList(argv[1]);

    float ctau_old = atof(argv[2]);
    float ctau_new = atof(argv[3]);
    ...
    for(auto& line : inputLines){
        ...
        double normalizationWeight = exp(1.0/ctau_old - 1.0/ctau_new);
       ...
       TFile *inputFile = TFile::Open(fileName.c_str(), "READ");
        ...
        while((key = (TKey*)nextkey())){
            ...
            float weight = 1;
              ...
              inputTree->SetBranchAddress("ctau", &ctau);
      	    }
            ...
            for (int n=0;n<inputTree->GetEntries();n++) {
                ...
                if(normalizationWeight >= 0){
                    ...
                    normalizationWeight = exp(ctau * (1.0/ctau_old - 1.0/ctau_new));
