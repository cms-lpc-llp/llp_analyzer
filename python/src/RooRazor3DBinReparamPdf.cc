...
#include "RooRazor3DBinReparamPdf.h"
#include "RooRealVar.h"

using namespace std;

ClassImp(RooRazor3DBinReparamPdf)
//---------------------------------------------------------------------------
RooRazor3DBinReparamPdf::RooRazor3DBinReparamPdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
...
//TH3* _Hnominal) : RooAbsPdf(name, title), 
  ...
  AR("AR", "AR Offset", this, _ar),
  BR("BR", "BR Offset", this, _br),
...
//---------------------------------------------------------------------------
RooRazor3DBinReparamPdf::RooRazor3DBinReparamPdf(const RooRazor3DBinReparamPdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooRazor3DBinReparamPdf::setTH3Binning(TH3* _Hnominal){
...
//---------------------------------------------------------------------------
Double_t RooRazor3DBinReparamPdf::evaluate() const
{
  Double_t integral = 0.0;
  Double_t total_integral = 1.0;

  if(B <= 0. || N <= 0. || AR <= 0. || BR <= 0.) return 0.0;
...
// //---------------------------------------------------------------------------
Int_t RooRazor3DBinReparamPdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooRazor3DBinReparamPdf::analyticalIntegral(Int_t code, const char* rangeName) const{
   ...
   if(code==1) { 
     ...
     cout << "WARNING IN RooRazor3DBinReparamPdf: integration code is not correct" << endl;
