...
#include "RooRazor3DBinMRSlopePdf.h"
#include "RooRealVar.h"

using namespace std;

ClassImp(RooRazor3DBinMRSlopePdf)
//---------------------------------------------------------------------------
RooRazor3DBinMRSlopePdf::RooRazor3DBinMRSlopePdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
...
//---------------------------------------------------------------------------
RooRazor3DBinMRSlopePdf::RooRazor3DBinMRSlopePdf(const RooRazor3DBinMRSlopePdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooRazor3DBinMRSlopePdf::setTH3Binning(TH3* _Hnominal){
...
//---------------------------------------------------------------------------
Double_t RooRazor3DBinMRSlopePdf::evaluate() const
{
  ...
  if (total_integral>0.0) {
    return histArray[iBin] * suppress * integral;
  } else return 0;

}

// //---------------------------------------------------------------------------
Int_t RooRazor3DBinMRSlopePdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooRazor3DBinMRSlopePdf::analyticalIntegral(Int_t code, const char* rangeName) const{
   ...
   if(code==1) { 
	 ...
	 if (zCut >= zLow and zCut < zHigh){
	   ...
	   if(xHigh <= xCut && yHigh <= yCut) integral += 0.0;
	     ...
	     integral += histArray[iBin] * suppress * ( Gfun(xCut,yLow)-Gfun(xCut,yHigh)-Gfun(xHigh,yLow)+Gfun(xHigh,yHigh) );
	   }
	   else if(yLow < yCut && yHigh > yCut && xHigh <= xCut) {
	     integral += histArray[iBin] * suppress * ( Gfun(xLow,yCut)-Gfun(xLow,yHigh)-Gfun(xHigh,yCut)+Gfun(xHigh,yHigh) );
	   }
	   else if(xLow < xCut && xHigh > xCut && yLow < yCut && yHigh > yCut) {
	     integral += histArray[iBin] * suppress * ( -Gfun(xLow,yHigh)-Gfun(xHigh,yLow)+Gfun(xHigh,yHigh)+Gfun(xLow,yCut)+Gfun(xCut,yLow)-Gfun(xCut,yCut) );
	   }
	   else integral += histArray[iBin] * suppress * ( Gfun(xLow,yLow)-Gfun(xLow,yHigh)-Gfun(xHigh,yLow)+Gfun(xHigh,yHigh) );
	 }
       }
     }
   } else {
     cout << "WARNING IN RooRazor3DBinMRSlopePdf: integration code is not correct" << endl;
